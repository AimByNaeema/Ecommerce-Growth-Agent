...
or a performance figure. It supports 10 capabilities, returning one common structured
result shape,
...
Capability → schema mapping (**zero new schema surface beyond the 4 dedicated
...
`platformContentModel.js` all carry their own `evidence` array field (like Marketing's
schemas, unlike SEO's/Listing's) — so evidence is assigned directly from
...
facebook | tiktok | pinterest | youtube` for social content and content generation;
`meta_ads | google_ads | tiktok_ads` for ad campaigns; the union of both 8 for
strategy's platform selection) — no platform beyond these 8 is a valid capability
today, matching the explicit scope of this build (no unnecessary platform integrations
added).

Four tool ids (`tools/toolRegistry.js`) are wired to this agent, mirroring the task's
...
[`tools/socialMediaStrategyTool.js`](../tools/socialMediaStrategyTool.js) and
[`tools/platformContentTool.js`](../tools/platformContentTool.js) each wrap a single
capability (no platform-dispatch key needed — `platformContentTool.js`'s `platform` is
a required field on the record itself). All four return `{ status, result, error }`
with `status` one of `success`, `empty`, `partial`, or `failed` — never fabricating a
result when structured input is missing or incomplete. The Social & Advertising
specialist is fully wired into the Chief/Orchestrator
