...
product status, collections, and metafields, in one GraphQL round-trip). Read-only only
— no write/mutation of any kind exists here (orders, product writes, etc. are still
untouched), and it is not wired into `agent/core/agentContract.js`'s stages yet — that
...
built-in `process.loadEnvFile`. A missing config, a network failure, or a
...
are set, confirm the connection and print up to 5 real products against the real store.
