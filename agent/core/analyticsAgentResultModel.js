...
// The compact shape of one Analytics & Optimization Agent result - the structured
// envelope agent/core/analyticsAgent.js returns for any of its 10 supported
// capabilities. Schema and a couple of pure helpers only - no lookup/search/synthesis
// logic, matching every other *AgentResultModel.js file in agent/core/ and mirroring
// agent/core/marketingAgentResultModel.js's own design exactly.
//
// This wraps existing per-capability records (agent/core/analyticsModel.js,
// agent/core/growthOpportunityModel.js) rather than duplicating their fields - see
...
const ANALYTICS_AGENT_RESULT_FIELDS = [
  {
    id: 'capability',
    title: 'Capability',
    type: `enum: ${ANALYTICS_CAPABILITIES.join(' | ')}`,
    description: 'Which of the Analytics & Optimization Agent\'s supported capabilities this result is for.',
  },
  {
    id: 'topic',
    title: 'Topic',
  ...
  },
  {
    ...
    title: 'Source',
    type: 'array',
    description: 'Flattened reference/source entries (e.g. an analytics dashboard export, a data export) drawn from the underlying specialized_records.',
  },
  {
    id: 'confidence',
    title: 'Confidence',
  ...
  },
  {
    ...
    title: 'Limitations',
    type: 'array',
    description: 'Honest gaps/caveats about this result (e.g. no live analytics provider configured, missing evidence) - always populated, never omitted.',
  },
  {
    id: 'recommendations',
  ...
  },
  {
    ...
    type: 'array',
    description: 'The underlying per-capability model record(s) this result was composed from (e.g. agent/core/analyticsModel.js snapshot records, agent/core/growthOpportunityModel.js records, agent/core/insightModel.js records) - so this envelope wraps existing schemas rather than duplicating them.',
  },
];

const ARRAY_FIELD_IDS = ANALYTICS_AGENT_RESULT_FIELDS.filter((field) => field.type === 'array').map(
  (field) => field.id
);

// Returns a blank Analytics agent result conforming to ANALYTICS_AGENT_RESULT_FIELDS.
// No real analysis - callers (agent/core/analyticsAgent.js) fill it in.
function createEmptyAnalyticsAgentResult(capability = null, topic = '') {
  return {
    capability,
    topic,
    market: '',
...
// Checks that an Analytics agent result has exactly the expected keys, with the
// expected basic shapes. Does not guess or fill in anything missing - only reports.
function validateAnalyticsAgentResultShape(record) {
  const errors = [];
  ...
  const expectedIds = ANALYTICS_AGENT_RESULT_FIELDS.map((field) => field.id);
  ...
  if ('capability' in record && !ANALYTICS_CAPABILITIES.includes(record.capability)) {
    errors.push(`capability must be one of: ${ANALYTICS_CAPABILITIES.join(', ')}`);
  }
...
if (require.main === module) {
  console.log('Smart E-Commerce Growth AI Agent - Analytics & Optimization agent result model (schema only):\n');
  ANALYTICS_AGENT_RESULT_FIELDS.forEach((field, index) => {
    console.log(`${index + 1}. [${field.id}] ${field.title} (${field.type})`);
    console.log(`   ${field.description}`);
  });
  console.log('\nExample empty result:');
  console.log(JSON.stringify(createEmptyAnalyticsAgentResult('sales', '(no topic set)'), null, 2));
}
